    ...
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await edit_or_reply(event,
                                ...
                                " untuk dapat deploy perubahan terbaru dari ⚡ᴀʙɪɴɢ-υѕєявσт⚡.`"
                                )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            await edit_delete(event,
                              f"{txt}\n`Kredensial Heroku tidak valid untuk deploy Abing-Project dyno.`"
                              )
            return repo.__del__()
        await edit_or_reply(event,
        ...
        if build.status == "failed":
            ...
            await edit_delete(event,
                              "`Abing-Userbot Berhasil DiUpdate🛃,Restart Tunggu Sebentar`"
                              )

        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, "#BOT \n" "`Abing-Userbot Berhasil Di Update`"
            )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    x = await edit_or_reply(event, "**⚡ᴀʙɪɴɢ-υѕєявσт⚡** `Berhasil Di Update!`")
    await asyncio.sleep(1)
    await x.edit("**⚡ᴀʙɪɴɢ-υѕєявσт⚡** `Di Restart....`")
    await asyncio.sleep(1)
...
async def upstream(event):
    ...
    if changelog == "" and force_update is False:
        await xx.edit(
            f"\n⚡ᴀʙɪɴɢ-υѕєявσт⚡ Sudah Versi Terbaru || Tunggu Update Terbaru\n"
        )
    ...
    if force_update:
        ...
        await xx.edit("` Proses Update ⚡ᴀʙɪɴɢ-υѕєявσт⚡, Loading....1%`")
        await xx.edit("` Proses Update ⚡ᴀʙɪɴɢ-υѕєявσт⚡ Loading....20%`")
        await xx.edit("` Proses Update ⚡ᴀʙɪɴɢ-υѕєявσт⚡, Loading....35%`")
        await xx.edit("` Proses Update ⚡ᴀʙɪɴɢ-υѕєявσт⚡, Loading....77%`")
        await xx.edit("` Proses Update ⚡ᴀʙɪɴɢ-υѕєявσт⚡, Updating...90%`")
        await xx.edit(
...
CMD_HELP.update(
    {
        ...
        "\n• : Untuk Melihat Pembaruan Terbaru Abing-Userbot."
        f"\n\n𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `{cmd}update now`"
        "\n• : Memperbarui Abing-Userbot."
        f"\n\n𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `{cmd}update deploy`"
        "\n• : Memperbarui Abing-Userbot Dengan Cara Men-Deploy Ulang."
    }
)
